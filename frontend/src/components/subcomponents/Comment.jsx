...
export default function Comment({ category, comment, replyTo, setReplyTo, postId, userId })
{
  ...
  return (
    ...
    <div className='flex flex-col'>
      ...
      {comment.replies > 0 &&
        <button 
          className='mt-2 ml-4 mr-auto text-sm'
          onClick={() => getReplies()}
        >
          {
      ...
      <div className='border-l ml-4 pl-4 mt-4 flex flex-col gap-4 dark:border-slate-700'>
        <Comments category={category} comments={replies} replyTo={replyTo} setReplyTo={setReplyTo} postId={postId}/>
      </div>
      }
    </>
  )
