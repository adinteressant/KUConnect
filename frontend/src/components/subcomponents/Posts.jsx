    ...
    try {
      ...
      if (response.ok) {
        setShowCommentBox((prev) =>
          prev.map((p) => p.postId === post._id ? {
            ...p, content: '', display: p.display.concat({
              pfp: updatedPost.pfp,
        ...
        <div className="mt-4">
          {posts.length > 0 ? (
            posts.map((post) => (
              <div
                key={post._id}
                className={`group/post relative bg-white dark:bg-slate-800 dark:shadow-black dark:text-slate-200 p-4 rounded-lg shadow-md mb-4 transition-all duration-300 ${post.isUpdating ? 'scale-105' : 'scale-100'
                  }`}
              >
            ...
            <div className="bg-white dark:bg-slate-800 dark:text-gray-200 p-4 rounded-lg shadow-md text-center">
              No posts available.
            </div>
          )}
        </div>
