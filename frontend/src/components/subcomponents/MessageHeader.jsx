...
export default function MessageHeader({ username, userId }) {
  ...
  useEffect(() => {
    if (!userId) {
      console.log('No userId passed');  // Log if no userId
      setLoading(false);
      return;
    }
    ...
    const loadProfileData = async () => {
      try {
        ...
        const response = await fetch(`/api/get-profile?id=${userId}`);
        if (!response.ok) {
