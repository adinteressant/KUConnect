...
const OnePost = () => {
  ...
  const handleLike = async(post) => {
    ...
    try {
      ...
      if(response.ok) {
        setLiked((prev) => !prev);
        setPost({ ...updatedPost.post, isUpdating: true });
        
        // Reset the isUpdating state after animation
        setTimeout(() => {
          setPost({ ...updatedPost.post, isUpdating: false });
        }, 300);
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <div
        className={`group/post relative bg-white p-4 rounded-lg shadow-md transition-all duration-300 ${
      ...
      >
        <div className='flex gap-2 items-center'>
          ...
          <div>
            <Link to={`/${post.username}`} className="text-gray-800 font-semibold">
              {post.username}
            </Link>
 ...
 <hr className='absolute left-0 right-0 mt-4'/>
        ...
        {/* Rest of the component including likes, comments, shares information */}
        {/* Like, Comment, Share buttons */}
        {/* Comment section */}
        {/* Everything exactly the same as in your Posts component */}
        {/* ... */}
        ...
        {(post.userId === userProfile.user_id) &&
          <div className='absolute top-3 right-3 flex flex-col items-end' onMouseLeave={() => setPostOptions(() => '')}>
            <button className='hidden group-hover/post:block rounded-full p-1 transition-all duration-300 hover:bg-gray-100' 
                    onClick={() => setPostOptions((prev) => (prev === post._id) ? '' : post._id)}>
              <svg width="16" height="16" viewBox="0 0 24 24" className='stroke-1 stroke-gray-600'>
            ...
            {postOptions === post._id &&
              <div className='rounded-lg shadow-lg'>  
                <button onClick={() => deletePost(post)} 
                        className='flex items-center gap-2 rounded-lg p-2 text-red-400 hover:bg-gray-100 transition-all duration-300'>
                  <svg className='stroke-red-400 fill-none stroke-2' width="20" height="20" viewBox="0 0 24 24">
                    <path d="M3 6h18"/>
      ...
      {/* Like Overlay */}
      {showLikeOverlay && (
        <div className={`w-screen h-screen fixed inset-0 z-50 flex items-center justify-center bg-black transition-all duration-300 ${overlayTransitionState ? 'bg-opacity-50' : 'bg-opacity-0'}`}      
             onClick={closeLikeOverlay}>
          <div onClick={(e) => e.stopPropagation()} 
               className={`relative bg-white w-80 h-96 rounded-lg shadow-2xl transition-all duration-300 ${overlayTransitionState ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
            <button onClick={closeLikeOverlay} className='absolute top-2 right-2 p-2 rounded-full hover:bg-gray-200 transition-all duration-300'>
              <svg width='24' height='24' viewBox='0 0 24 24' className='stroke-gray-600 fill-none'>
      ...
      {/* Comment Overlay */}
      {showCommentOverlay && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black transition-all duration-300 ${overlayTransitionState ? 'bg-opacity-50' : 'bg-opacity-0'}`}      
             onClick={closeCommentOverlay}>
          <div onClick={(e) => e.stopPropagation()} 
               className={`relative bg-white min-w-80 w-[50%] h-[60%] rounded-lg shadow-2xl transition-all duration-300 ${overlayTransitionState ? 'opacity-100 scale-100' : 'opacity-0 scale-50'}`}>
